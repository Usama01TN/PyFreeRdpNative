...
build_freerdp.py - fetch + compile FreeRDP (client + server), copy artifacts
                   into pyfreerdpnative/_libs/.

Usage:
    python -m pyfreerdpnative.scripts.build_freerdp [--ref v3.31.1]
                                              [--prefix /opt/freerdp]
...
def cmake_options_for(profile, host_os, enable_channels=None,
    ...
    if host_os == "Linux":
        ...
        opts += [
            ...
            # can be moved into pyfreerdpnative/_libs and bundled by delocate.
...
def _patch_msvc_static_crt_guard(src):
    ...
    if guard not in text:
        if "pyfreerdpnative: static CRT in a shared build" in text:
            return                                   # already patched
        print("[patch] MSVCRuntime.cmake: guard not found, skipping "
              "(upstream may have changed it)")
        return
    replacement = (
        '    # pyfreerdpnative: static CRT in a shared build is allowed when every\n'
...
def patch_source_tree(src, host_os, profile, windows_shadow=None):
    ...
    if anchor not in text:
        ...
        return
    note = (
        "# pyfreerdpnative: win_shadow.c / win_wds.c use the SDK's nameless unions"
...
def _fix_rpath(out_dir):
    ...
    the files sit flat in pyfreerdpnative/_libs/: dlopen('.../_libs/
...
def verify_loadable(out_dir, profile, arch="host"):
    """
    Do exactly what pyfreerdpnative's tests do: ctypes-load the staged client and
...
def stage_executables(prefixes, libs_dir, deps_prefix=None, arch="host"):
    ...
    pyfreerdpnative/_bin/ (Windows: into _libs/, where the DLLs are - Windows
...
def soname_only(artifacts):
    ...
    the dynamic loader resolves dependencies by SONAME and pyfreerdpnative's
...
def install_into_package(artifacts, arch="host", deps_prefix=None,
    ...
    out = os.path.join(package_root(), "pyfreerdpnative", "_libs")
    if not os.path.isdir(out):
        os.makedirs(out)
    count = 0
    module_dirs = set()
...
def build_android(src, abi, api_level, jobs, profile, enable_channels=None,
    ...
    target = os.path.join(package_root(), "pyfreerdpnative", "_libs", "android", abi)
...
def build_ios(src, jobs, profile, enable_channels=None,
    ...
    if deps_prefix:
        ...
        if not ios_static and deps_has_openh264(deps_prefix):
            ...
            hook = os.path.join(build_dir, "pyfreerdpnative-link-libcxx.cmake")
            with open(hook, "w") as fh:
                fh.write("# pyfreerdpnative: OpenH264 (C++) is linked statically into the\n"
    ...
    target = os.path.join(package_root(), "pyfreerdpnative", "_libs", "ios",
                          ios_platform)
    if not os.path.isdir(target):
        os.makedirs(target)
    staged = []
...
def resolve_deps_prefix(args):
    ...
    if os.path.isdir(cand):
        return cand
    print("[pyfreerdpnative-build] --deps-prefix auto: {0} not found".format(cand))
    return None
...
# not consume pyfreerdpnative/_libs or build/deps, so they are separate targets.
...
def ensure_apk_keystore(store, alias, store_pw, key_pw):
    ...
    run(["keytool", "-genkeypair", "-v", "-keystore", store,
         ...
         "-dname", "CN=pyfreerdpnative, OU=CI, O=pyfreerdpnative, C=US"])
    return store
    ...
    if build_type == "Release":
        store = ensure_apk_keystore(
            keystore or os.path.join(repo_root(), "build", "android-apk",
                                     "pyfreerdpnative-release.keystore"),
...
def resolve_source(args):
    ...
    return fetch_source(args.ref, os.path.join(tempfile.gettempdir(),
                                               "pyfreerdpnative-build"))


def main():
    p = argparse.ArgumentParser(description="Build FreeRDP for pyfreerdpnative")
    ...
    p.add_argument("--deps-prefix", metavar="DIR",
                   help="Prefix produced by scripts/build_deps.py (FFmpeg, "
                        ...
                        "pyfreerdpnative/_libs. 'auto' = build/deps/<label> if "
                        "it exists.")
    ...
    p.add_argument("--with-executables", action="store_true",
                   help="Also build and stage the executables (xfreerdp/"
                        ...
                        "deps prefix) into pyfreerdpnative/_bin (Windows: _libs).")
    ...
    if args.require_version is not None:
        if args.require_version != BUILD_SCRIPT_VERSION:
            ...
            return 2
        print("[pyfreerdpnative-build] build script version {0} OK".format(
            BUILD_SCRIPT_VERSION))
        return 0

    print("[pyfreerdpnative-build] target={0} profile={1} ref={2} jobs={3}".format(
    ...
    print("[pyfreerdpnative-build] edition: {0}".format(args.edition))
    ...
    if deps_prefix:
        print("[pyfreerdpnative-build] deps prefix: {0}".format(deps_prefix))
    ...
    if channels_enabled:
        rows = resolve_channels(args.profile, host_os,
                                args.enable_channel, args.disable_channel)
        print("[pyfreerdpnative-build] client channels: {0}".format(
            ", ".join(n for n, on, c, s in rows if c) or "none"))
        print("[pyfreerdpnative-build] server channels: {0}".format(
            ", ".join(n for n, on, c, s in rows if s) or "none"))
    else:
        print("[pyfreerdpnative-build] channels: disabled (--no-channels)")
